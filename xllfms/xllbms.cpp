...
static AddInX xai_option_bms_value(
	...
	.Arg(XLL_DOUBLEX, _T("Rate"), _T("is the riskless rate."))
	.Arg(XLL_DOUBLEX, _T("Stock"), _T("is the underlying stock value."))
	...
	.Category(CATEGORY)
	.FunctionHelp(_T("Return 5 column array of Black-Merton-Scholes option value, delta, gamma, vega, and theta."))
	);
xfp* WINAPI xll_option_bms_value(double r, double f, double sigma, double k, double t)
{
#pragma XLLEXPORT
	static FPX o(1, 5);
	...
	try {
		o = 0;
		o[0] = bms::value(r, f, sigma, k, t, &o[1], &o[2], &o[3], &o[4]);
	}
